...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import ReSHOP_jll on Julia versions older than 1.3!")
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("b1e335f6-4372-40fd-9431-5a5c4704c04b"))
platforms = [Pkg.Artifacts.unpack_platform(e, "ReSHOP", artifacts_toml) for e in artifacts["ReSHOP"]]
...
if best_platform === nothing
    @debug("Unable to load ReSHOP; unsupported platform $(triplet(platform_key_abi()))")
else
    ...
    if Sys.isapple(platform_key_abi())
        include(joinpath(@__DIR__, "apple.jl"))
    elseif Sys.iswindows(platform_key_abi())
        include(joinpath(@__DIR__, "windows.jl"))
    elseif Sys.islinux(platform_key_abi())
        include(joinpath(@__DIR__, "linux.jl"))
    else
        @debug("Unable to load ReSHOP; unsupported platform $(triplet(platform_key_abi()))")
    end
end

end  # module ReSHOP_jll
